...
analysis_sido <- analysis_total %>% filter(substr(일시,1,7)=='2017-12'|substr(일시,1,7)=='2018-01'|substr(일시,1,7)=='2018-02') %>% group_by(시도코드,시도) %>% summarise(`평균기온(°C)` = mean(`평균기온(°C)`,na.rm=TRUE),
...
# analysis_sido$SO2 <- analysis_sido$SO2 * 100
# analysis_sido$NO2 <- analysis_sido$NO2 * 100
# analysis_sido$O3 <- analysis_sido$O3 * 100
```
