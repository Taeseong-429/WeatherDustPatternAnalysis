...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("대구")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("대구")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("대구"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("대구")) 
...
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + `최고기온(°C)` + `평균 풍속(m/s)` + 
    `일 최심신적설(cm)` + `강수 계속시간(hr)` + CO + NO2 + PM10 + 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + `평균기온(°C)`, data =analysis_2016_quarter4)
