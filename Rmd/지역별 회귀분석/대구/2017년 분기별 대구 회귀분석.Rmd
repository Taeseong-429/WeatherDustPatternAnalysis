...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("대구")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("대구")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("대구"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("대구")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `일 최심신적설(cm)` + `평균 풍속(m/s)` + 
    NO2 + `일강수량(mm)` + `평균 현지기압(hPa)` + `평균기온(°C)`, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + NO2 + CO + `최저기온(°C)`, data = analysis_2017_quarter2)
...
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + `최저기온(°C)` + 
    `최고기온(°C)` + PM25, data =analysis_2017_quarter4)
