...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("대구")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("대구")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("대구"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("대구")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ SO2 + CO + NO2 + `평균 풍속(m/s)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + PM25 + `최저기온(°C)` + `평균 풍속(m/s)`, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + O3 + `최저기온(°C)` + 
    `강수 계속시간(hr)` + `평균기온(°C)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + `최저기온(°C)` + 
    PM25 + `최고기온(°C)`, data =analysis_2018_quarter4)
