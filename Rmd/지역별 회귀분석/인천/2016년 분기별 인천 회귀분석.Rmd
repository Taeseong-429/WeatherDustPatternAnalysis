...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("인천")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("인천")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("인천"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("인천")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `일 최심신적설(cm)` + PM25 + NO2 + PM10, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + CO, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + O3 + `강수 계속시간(hr)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + PM25 + PM10 + `최저기온(°C)` + `평균 현지기압(hPa)` + 
    CO + `일강수량(mm)`, data =analysis_2016_quarter4)
