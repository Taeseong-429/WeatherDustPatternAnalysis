...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("인천")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("인천")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("인천"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("인천")) 
...
fit <-  lm(formula = 발병률 ~ `강수 계속시간(hr)` + `평균 풍속(m/s)` + 
    NO2 + `최고기온(°C)` + `일강수량(mm)`, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `최저기온(°C)` + `최고기온(°C)` + `강수 계속시간(hr)` + 
    SO2 + NO2, data = analysis_2017_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 현지기압(hPa)`, data =analysis_2017_quarter4)
