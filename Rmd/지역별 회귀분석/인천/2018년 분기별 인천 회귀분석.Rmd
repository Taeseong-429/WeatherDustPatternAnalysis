...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("인천")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("인천")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("인천"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("인천")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `최고기온(°C)` + SO2 + `평균 풍속(m/s)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `강수 계속시간(hr)` + O3 + `평균기온(°C)` + 
    `평균 풍속(m/s)` + NO2 + SO2, data = analysis_2018_quarter2)
...
fit<- lm(formula = 발병률 ~ `평균기온(°C)` + `최저기온(°C)` + `일강수량(mm)` + 
    `강수 계속시간(hr)` + SO2 + CO + NO2, data =analysis_2018_quarter4)
