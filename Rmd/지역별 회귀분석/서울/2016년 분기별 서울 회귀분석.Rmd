...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("서울")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("서울")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("서울"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("서울")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ PM25 + NO2 + PM10 + `일 최심신적설(cm)`, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + SO2, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + O3 + SO2, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + PM25, data =analysis_2016_quarter4)
