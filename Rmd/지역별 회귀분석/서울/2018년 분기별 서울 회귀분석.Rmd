...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("서울")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("서울")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("서울"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("서울")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + `최고기온(°C)` + `평균 풍속(m/s)` + 
    CO, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `평균 풍속(m/s)` + NO2 + CO, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + O3 + `평균 풍속(m/s)` + PM25 + `평균 현지기압(hPa)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)`, data =analysis_2018_quarter4)
