...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("경남")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("경남")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("경남"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("경남")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `일 최심신적설(cm)` + PM25 + NO2 + `평균 풍속(m/s)` + 
    CO + O3, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + PM10, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + CO + `최저기온(°C)` + 
    SO2 + `강수 계속시간(hr)` + `평균 현지기압(hPa)`, 
