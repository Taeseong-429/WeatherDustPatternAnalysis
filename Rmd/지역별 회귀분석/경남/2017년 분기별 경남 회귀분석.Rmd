...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("경남")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("경남")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("경남"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("경남")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ PM25 + `평균 풍속(m/s)` + NO2 + CO + PM10 + 
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + SO2 + `평균 현지기압(hPa)`, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)` + `최저기온(°C)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO, data =analysis_2017_quarter4)
