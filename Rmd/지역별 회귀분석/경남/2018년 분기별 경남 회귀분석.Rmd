...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("경남")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("경남")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("경남"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("경남")) 
...
fitdata <- analysis_2018_quarter1 %>% dplyr::select(-`일 최심신적설(cm)`)
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + NO2 + CO, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ PM10 + NO2 + CO + `평균 풍속(m/s)`, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + `일강수량(mm)` + 
    SO2 + `최저기온(°C)` + `평균기온(°C)` + CO, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + `최저기온(°C)` + 
    `평균 현지기압(hPa)`, data =analysis_2018_quarter4)
