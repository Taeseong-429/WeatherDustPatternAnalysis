...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("부산")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("부산")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("부산"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("부산")) 
...
fitdata <- analysis_2017_quarter1 %>% dplyr::select(-`일 최심신적설(cm)`)
...
```{r}
fit <-  lm(formula = 발병률 ~ PM25 + SO2 + NO2 + `평균 풍속(m/s)` + `평균기온(°C)` + 
    `최저기온(°C)` + CO, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ SO2 + CO + NO2, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + SO2 + `최저기온(°C)` + `평균 풍속(m/s)` + 
    PM25, data =analysis_2017_quarter4)
