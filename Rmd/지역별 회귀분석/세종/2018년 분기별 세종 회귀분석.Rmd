...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + CO + `평균 풍속(m/s)` + `일강수량(mm)` + 
    NO2 + `강수 계속시간(hr)` + `평균 현지기압(hPa)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `최고기온(°C)` + `평균 풍속(m/s)` + NO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `최고기온(°C)` + O3 + `평균 풍속(m/s)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ `평균기온(°C)` + `최고기온(°C)` + `평균 풍속(m/s)` + 
    SO2 + NO2 + PM25, data =analysis_2018_quarter4)
