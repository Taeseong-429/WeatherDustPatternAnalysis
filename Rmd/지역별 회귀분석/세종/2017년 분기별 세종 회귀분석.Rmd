...
```{r}
fit <-  lm(formula = 발병률 ~ `최저기온(°C)` + PM10 + SO2 + `평균 현지기압(hPa)` + 
    `평균 풍속(m/s)` + NO2 + O3, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)`, data = analysis_2017_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `최고기온(°C)` + CO + `평균 현지기압(hPa)`, data =analysis_2017_quarter4)
