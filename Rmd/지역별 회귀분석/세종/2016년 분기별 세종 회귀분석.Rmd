...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("세종")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("세종")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("세종"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("세종")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ PM25 + NO2 + `평균 풍속(m/s)` + O3, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + PM10 + CO, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ `평균기온(°C)` + `최고기온(°C)` + `평균 현지기압(hPa)` + 
    `일강수량(mm)` + SO2 + NO2 + PM10, 
...
```{r}
fit<- lm(formula = 발병률 ~ SO2 + `평균 풍속(m/s)` + NO2 + PM25 + O3, data =analysis_2016_quarter4)
