...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("경기")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("경기")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("경기"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("경기")) 
...
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    CO + PM10 + NO2 + `일 최심신적설(cm)`, data = analysis_2018_quarter1)
...
fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + PM25 + `최저기온(°C)` + 
    SO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + O3 + `평균 풍속(m/s)` + `최저기온(°C)` + 
    PM25 + `평균기온(°C)` + `강수 계속시간(hr)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + `최저기온(°C)`, data =analysis_2018_quarter4)
