...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("경기")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("경기")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("경기"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("경기")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ PM25 + `평균 풍속(m/s)` + NO2, data = analysis_2016_quarter1)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + CO + `평균 현지기압(hPa)`, data =analysis_2016_quarter4)
