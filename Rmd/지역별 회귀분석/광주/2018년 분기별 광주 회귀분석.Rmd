...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("광주")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("광주")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("광주"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("광주")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균 풍속(m/s)` + SO2 + `최고기온(°C)` + 
    NO2 + CO + PM10 + `강수 계속시간(hr)` + `일강수량(mm)` + 
    `최저기온(°C)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + PM25 + SO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + O3 + `평균 풍속(m/s)` + `강수 계속시간(hr)` + 
    SO2 + `평균 현지기압(hPa)` + `평균기온(°C)` + `최고기온(°C)` + 
    `최저기온(°C)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + PM25 + `최저기온(°C)` + 
    `강수 계속시간(hr)`, data =analysis_2018_quarter4)
