...
```{r}
fit <-  lm(formula = 발병률 ~ `평균 풍속(m/s)` + NO2 + CO + `최고기온(°C)`, data = analysis_2018_quarter1)
...
fit <- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + `최저기온(°C)` + 
    SO2 + PM10, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 현지기압(hPa)` + O3 + `평균 풍속(m/s)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + PM25 + `평균 풍속(m/s)` + `최저기온(°C)` + 
    `평균 현지기압(hPa)` + SO2, data =analysis_2018_quarter4)
