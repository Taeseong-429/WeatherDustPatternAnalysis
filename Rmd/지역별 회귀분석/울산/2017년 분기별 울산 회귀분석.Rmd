...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("울산")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("울산")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("울산"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("울산")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + SO2, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `평균기온(°C)` + `최저기온(°C)` + `평균 풍속(m/s)` + 
    `평균 현지기압(hPa)` + SO2 + CO + NO2, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + PM10 + SO2 + O3, 
...
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 현지기압(hPa)` + SO2 + 
    `최저기온(°C)`, data =analysis_2017_quarter4)
