...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("울산")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("울산")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("울산"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("울산")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균 풍속(m/s)` + NO2 + `최고기온(°C)`,data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `최저기온(°C)` + `최고기온(°C)` + `평균 풍속(m/s)` + 
    SO2 + CO + NO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)` + O3 + `일강수량(mm)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + `최저기온(°C)` + `평균 풍속(m/s)`, data =analysis_2018_quarter4)
