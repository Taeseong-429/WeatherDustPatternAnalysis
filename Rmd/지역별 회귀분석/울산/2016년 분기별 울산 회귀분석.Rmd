...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("울산")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("울산")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("울산"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("울산")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + PM25 + SO2 + 
    `최저기온(°C)` + `일강수량(mm)`, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + PM25 + `최고기온(°C)` + 
    `최저기온(°C)` + `강수 계속시간(hr)` + `평균 현지기압(hPa)`, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ O3 + NO2 + `평균 풍속(m/s)` + `강수 계속시간(hr)` + 
    SO2 + PM10 + PM25 + `최저기온(°C)` + `평균기온(°C)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + CO + `평균 현지기압(hPa)` + 
    SO2 + `최저기온(°C)`, data =analysis_2016_quarter4)
