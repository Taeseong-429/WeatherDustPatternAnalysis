...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("대전")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("대전")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("대전"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("대전")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + NO2 + CO + PM25 + 
    `일강수량(mm)` + SO2, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + CO + SO2 + `평균 풍속(m/s)` + `최고기온(°C)`, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + O3 + `최고기온(°C)` + 
    `최저기온(°C)` + `강수 계속시간(hr)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + PM25 + `강수 계속시간(hr)`, data =analysis_2018_quarter4)
