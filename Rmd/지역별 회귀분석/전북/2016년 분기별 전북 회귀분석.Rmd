...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("전북")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("전북")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("전북"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("전북")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + `강수 계속시간(hr)` + 
    `일 최심신적설(cm)` + CO + `최고기온(°C)` + `평균기온(°C)` + 
