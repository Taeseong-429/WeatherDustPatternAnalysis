...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("전남")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("전남")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("전남"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("전남")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + `최저기온(°C)` + `최고기온(°C)` + 
    `평균 풍속(m/s)` + `강수 계속시간(hr)` + SO2 + NO2, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ PM10 + NO2 + CO + SO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + O3 + SO2 + CO + `강수 계속시간(hr)` + 
    `평균 풍속(m/s)` + PM25 + `평균 현지기압(hPa)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + SO2 + `최저기온(°C)` + PM25 + `평균 풍속(m/s)` + 
    `평균 현지기압(hPa)` + O3 + CO, data =analysis_2018_quarter4)
