...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("전남")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("전남")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("전남"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("전남")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균 풍속(m/s)` + NO2 + `평균 현지기압(hPa)`, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ SO2 + NO2 + PM25 + `평균 풍속(m/s)` + `최저기온(°C)`, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + SO2 + `강수 계속시간(hr)` + `평균 풍속(m/s)` + 
    PM25 + `일강수량(mm)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + SO2 + `최저기온(°C)` + CO, data =analysis_2017_quarter4)
