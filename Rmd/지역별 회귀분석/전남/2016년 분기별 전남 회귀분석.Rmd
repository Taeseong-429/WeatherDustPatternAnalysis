...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("전남")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("전남")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("전남"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("전남")) 
...
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + `최저기온(°C)` + `최고기온(°C)` + 
    `평균 풍속(m/s)` + `강수 계속시간(hr)` + SO2 + O3 + NO2 + 
    PM25, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + PM10 + SO2, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + SO2 + `일강수량(mm)` + 
    `최저기온(°C)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ `평균 현지기압(hPa)` + CO + O3 + NO2, data =analysis_2016_quarter4)
