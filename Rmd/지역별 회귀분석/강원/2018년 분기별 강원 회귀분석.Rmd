...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("강원")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("강원")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("강원"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + `평균 현지기압(hPa)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `최저기온(°C)` + `평균 풍속(m/s)` + SO2 + 
    CO + O3 + NO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + O3 + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    `최고기온(°C)` + `일강수량(mm)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ `최저기온(°C)` + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    CO + NO2, data =analysis_2018_quarter4)
