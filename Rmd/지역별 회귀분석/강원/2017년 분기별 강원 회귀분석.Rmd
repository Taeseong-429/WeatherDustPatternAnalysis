...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("강원")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("강원")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("강원"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + PM10 + PM25 + 
    `평균 현지기압(hPa)` + `최저기온(°C)`, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ O3 + SO2 + NO2 + `평균 풍속(m/s)` + CO, data = analysis_2017_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + O3 + `최저기온(°C)` + `평균기온(°C)` + 
    PM10, data =analysis_2017_quarter4)
