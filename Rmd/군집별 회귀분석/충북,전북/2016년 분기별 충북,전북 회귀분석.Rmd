...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("충북","전북")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("충북","전북")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("충북","전북"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("충북","전북")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ PM25 + NO2 + O3 + `평균 풍속(m/s)` + `평균기온(°C)` + 
    CO + `일강수량(mm)`, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + CO + `평균 현지기압(hPa)`, data = analysis_2016_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + CO + SO2, data =analysis_2016_quarter4)
