...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("제주")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("제주")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("제주"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("제주")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    `일강수량(mm)` + SO2 + CO + NO2 + PM25, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + SO2 + `평균기온(°C)`, data = analysis_2017_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균기온(°C)`, data =analysis_2017_quarter4)
