...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("울산","부산","전남")) 

analysis_2018_quarter1 <- analysis_2018_quarter1 %>% dplyr::select(-일시,-시도,-년도,-인구수,-`최다풍향(16방위)`,-발생건수, -`일 최심신적설(cm)`,-`강수 계속시간(hr)`,-`최고기온(°C)`,-`최저기온(°C)`)
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("울산","부산","전남")) 


analysis_2018_quarter2 <- analysis_2018_quarter2 %>% dplyr::select(-일시,-시도,-년도,-인구수,-`최다풍향(16방위)`,-발생건수, -`일 최심신적설(cm)`,-`강수 계속시간(hr)`,-`최고기온(°C)`,-`최저기온(°C)`)
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("울산","부산","전남"))

analysis_2018_quarter3 <- analysis_2018_quarter3 %>% dplyr::select(-일시,-시도,-년도,-인구수,-`최다풍향(16방위)`,-발생건수, -`일 최심신적설(cm)`,-`강수 계속시간(hr)`,-`최고기온(°C)`,-`최저기온(°C)`)
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("울산","부산","전남")) 

analysis_2018_quarter4 <- analysis_2018_quarter4 %>% dplyr::select(-일시,-시도,-년도,-인구수,-`최다풍향(16방위)`,-발생건수, -`일 최심신적설(cm)`,-`강수 계속시간(hr)`,-`최고기온(°C)`,-`최저기온(°C)`)
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + CO + SO2 + `일강수량(mm)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + SO2 + PM25 + `일강수량(mm)`, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + SO2 + `일강수량(mm)` + 
    O3 + `평균 현지기압(hPa)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + SO2 + `평균 풍속(m/s)` + CO + PM25 + 
    `평균 현지기압(hPa)` + PM10, data =analysis_2018_quarter4)
