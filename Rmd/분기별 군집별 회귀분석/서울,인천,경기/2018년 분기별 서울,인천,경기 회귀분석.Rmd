...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("서울","인천","경기")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("서울","인천","경기")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("서울","인천","경기"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("서울","인천","경기")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + `일강수량(mm)` + 
    `평균기온(°C)` + SO2 + NO2 + PM10 + CO, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + CO + PM25 + SO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ `평균기온(°C)` + NO2 + CO + O3 + PM10 + 
    PM25 + `평균 현지기압(hPa)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ `평균기온(°C)` + NO2 + `평균 풍속(m/s)` + 
