...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("서울","인천","경기")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("서울","인천","경기")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("서울","인천","경기"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("서울","인천","경기")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + O3 + NO2 + `평균 풍속(m/s)` + 
...
```{r}
fit <- lm(formula = 발병률 ~ `평균 풍속(m/s)` + SO2 + NO2 + PM25, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ `일강수량(mm)` + `평균기온(°C)` + NO2, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + `평균 현지기압(hPa)` + 
    CO + O3 + SO2 + PM25 + `일강수량(mm)`, data =analysis_2017_quarter4)
