...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("제주")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("제주")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("제주"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("제주")) 
...
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    CO + NO2 + PM10 + PM25, data = analysis_2016_quarter1)
...
fit <- lm(formula = 발병률 ~ `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    CO, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + PM10 + PM25, 
...
```{r}
fit<- lm(fformula = 발병률 ~ `평균기온(°C)` + NO2, data =analysis_2016_quarter4)
