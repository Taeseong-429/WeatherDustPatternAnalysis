...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("대구","충남","경남")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("대구","충남","경남")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("대구","충남","경남"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("대구","충남","경남")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + `일강수량(mm)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + CO + `평균 풍속(m/s)` + O3, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + `일강수량(mm)` + 
    O3 + SO2 + `평균 풍속(m/s)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + SO2 + PM25 + `평균 현지기압(hPa)`, data =analysis_2018_quarter4)
