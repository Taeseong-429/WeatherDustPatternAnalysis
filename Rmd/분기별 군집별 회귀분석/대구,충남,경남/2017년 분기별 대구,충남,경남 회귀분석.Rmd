...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("대구","충남","경남")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("대구","충남","경남")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("대구","충남","경남"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("대구","충남","경남")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + CO + `일강수량(mm)` + 
    `평균 현지기압(hPa)` + `평균기온(°C)` + PM10 + PM25, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `평균 풍속(m/s)` + SO2 + NO2, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + `평균 풍속(m/s)` + 
    SO2, 
...
fit<- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + `평균 현지기압(hPa)` + 
    PM25 + O3, data =analysis_2017_quarter4)
