...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("울산","부산","전남")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("울산","부산","전남")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("울산","부산","전남"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("울산","부산","전남")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `일강수량(mm)` + `평균 풍속(m/s)` + 
    CO + `평균 현지기압(hPa)` + SO2 + NO2, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ SO2 + NO2, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + CO + `평균 풍속(m/s)` + 
    SO2 + PM25, 
