...
analysis_2016_quarter1 <- analysis_2016_quarter1 %>% filter(시도 %in% c("광주","대전")) 
...
analysis_2016_quarter2 <- analysis_2016_quarter2 %>% filter(시도 %in% c("광주","대전")) 
...
analysis_2016_quarter3 <- analysis_2016_quarter3 %>% filter(시도 %in% c("광주","대전"))
...
analysis_2016_quarter4 <- analysis_2016_quarter4 %>% filter(시도 %in% c("광주","대전")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ SO2 + PM25 + NO2 + `평균 풍속(m/s)` + CO + 
    `평균기온(°C)`, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + `평균기온(°C)` + `평균 풍속(m/s)`, data = analysis_2016_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + PM25 + `평균 풍속(m/s)` + CO, data =analysis_2016_quarter4)
