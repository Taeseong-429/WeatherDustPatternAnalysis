...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("경북","강원")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("경북","강원")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("경북","강원"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("경북","강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + `평균기온(°C)` + 
    SO2 + NO2 + `평균 현지기압(hPa)`, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ `일강수량(mm)` + `평균 현지기압(hPa)` + 
    SO2 + NO2 + `평균 풍속(m/s)`, data = analysis_2017_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ `평균기온(°C)` + `평균 풍속(m/s)` + `일강수량(mm)` + 
    SO2 + NO2 + PM10, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + CO + `평균 현지기압(hPa)` + `평균 풍속(m/s)` + 
    O3 + PM25 + SO2, data =analysis_2017_quarter4)
