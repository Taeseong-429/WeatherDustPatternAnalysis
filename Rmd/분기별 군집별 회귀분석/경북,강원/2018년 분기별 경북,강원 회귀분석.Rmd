...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("경북","강원")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("경북","강원")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("경북","강원"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("경북","강원")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + NO2 + SO2 + CO + 
    `일강수량(mm)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ O3 + `평균 현지기압(hPa)` + SO2 + NO2 + 
    `일강수량(mm)`, data = analysis_2018_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + CO + PM25 + SO2, data =analysis_2018_quarter4)
