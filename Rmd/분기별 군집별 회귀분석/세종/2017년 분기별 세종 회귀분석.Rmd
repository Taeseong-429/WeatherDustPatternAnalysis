...
analysis_2017_quarter1 <- analysis_2017_quarter1 %>% filter(시도 %in% c("세종")) 
...
analysis_2017_quarter2 <- analysis_2017_quarter2 %>% filter(시도 %in% c("세종")) 
...
analysis_2017_quarter3 <- analysis_2017_quarter3 %>% filter(시도 %in% c("세종"))
...
analysis_2017_quarter4 <- analysis_2017_quarter4 %>% filter(시도 %in% c("세종")) 
...
fit <-  lm(formula = 발병률 ~ `평균기온(°C)` + O3 + `평균 풍속(m/s)` + 
    `평균 현지기압(hPa)` + CO, data = analysis_2017_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + SO2 + O3, data = analysis_2017_quarter2)
...
```{r}
fit<- lm(formula = 발병률 ~ `평균기온(°C)` + NO2 + `평균 현지기압(hPa)` + 
    O3, data =analysis_2017_quarter4)
