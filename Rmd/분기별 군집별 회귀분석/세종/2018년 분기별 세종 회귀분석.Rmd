...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("세종")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("세종")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("세종"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("세종")) 
...
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + `일강수량(mm)` + 
    CO + `평균기온(°C)`, data = analysis_2018_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ SO2 + O3 + NO2, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ NO2 + CO + `평균기온(°C)` + `평균 현지기압(hPa)`, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + SO2, data =analysis_2018_quarter4)
