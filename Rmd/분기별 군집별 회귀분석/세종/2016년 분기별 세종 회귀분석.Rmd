...
```{r}
fit <-  lm(formula = 발병률 ~ `평균 풍속(m/s)` + CO + O3 + NO2 + PM25, data = analysis_2016_quarter1)
...
```{r}
fit <- lm(formula = 발병률 ~ NO2 + PM10 + `일강수량(mm)` + `평균 풍속(m/s)` + 
    PM25, data = analysis_2016_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ `평균기온(°C)` + NO2 + O3, 
...
```{r}
fit<- lm(formula = 발병률 ~ SO2 + NO2 + `평균 풍속(m/s)` + PM25, data =analysis_2016_quarter4)
