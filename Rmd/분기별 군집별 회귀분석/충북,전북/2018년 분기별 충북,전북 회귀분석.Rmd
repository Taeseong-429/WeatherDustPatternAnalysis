...
analysis_2018_quarter1 <- analysis_2018_quarter1 %>% filter(시도 %in% c("충북","전북")) 
...
analysis_2018_quarter2 <- analysis_2018_quarter2 %>% filter(시도 %in% c("충북","전북")) 
...
analysis_2018_quarter3 <- analysis_2018_quarter3 %>% filter(시도 %in% c("충북","전북"))
...
analysis_2018_quarter4 <- analysis_2018_quarter4 %>% filter(시도 %in% c("충북","전북")) 
...
```{r}
fit <-  lm(formula = 발병률 ~ O3 + `평균 풍속(m/s)` + `평균 현지기압(hPa)` + 
    `평균기온(°C)` + NO2 + CO, data = analysis_2018_quarter1)
...
fit <- lm(formula = 발병률 ~ NO2 + CO + `평균 현지기압(hPa)` + `평균 풍속(m/s)` + 
    `평균기온(°C)`, data = analysis_2018_quarter2)
...
```{r}
 fit <- lm(formula = 발병률 ~ SO2 + `평균기온(°C)` + NO2 + O3, 
...
```{r}
fit<- lm(formula = 발병률 ~ NO2 + `평균 풍속(m/s)` + `평균 현지기압(hPa)`, data =analysis_2018_quarter4)
